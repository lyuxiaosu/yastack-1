...
InstanceImpl::InstanceImpl(Options& options, Event::TimeSystem& time_system,
    ...
    : options_(options), time_system_(time_system), restarter_(restarter),
      ...
      dispatcher_((options.hostOnly() == 0) ?
              api_->allocateDispatcher(time_system) : api_->allocateDispatcherHostOnly(time_system)),
...
void InstanceImpl::flushStats() {
  ...
  stats_store_.mergeHistograms([this]() -> void {
    //ENVOY_LOG_MISC(debug, "tid [{}] merge_complete_cb() ",std::this_thread::get_id());
...
void InstanceImpl::updateAdminPort(Options& options, envoy::config::bootstrap::v2::Bootstrap &bootstrap) {
     ::envoy::config::bootstrap::v2::Admin  *admin      = bootstrap.mutable_admin();
     ::envoy::api::v2::core::Address        *addr       = admin->mutable_address();
     ::envoy::api::v2::core::SocketAddress  *sa         = addr->mutable_socket_address();

     sa->set_port_value(sa->port_value() + options.ffProcId());
}
...
void InstanceImpl::initialize(Options& options,
  ...
  // Admin listens on ((configured port) + proc_id)
  updateAdminPort(options, bootstrap_);
  ...
  listener_manager_.reset(
      ...
      // Pass the dispatcher we have (parent's dispatcher) instead of creating a new one.
      // If no dispatcher is passed to ListenerManagerImpl, it'll create a new one.
      new ListenerManagerImpl(*this, listener_component_factory_, worker_factory_, time_system_, dispatcher()));
#else
  ...
  guard_dog_.reset(
      new Server::GuardDogImpl(stats_store_, *config_, time_system_));

  /* Move initialization of workers here */
#if defined (__HAS_FF__)
  // We need the RunHelper to be available to call from InstanceImpl::shutdown() below, so
  // we save it as a member variable.
  run_helper_ = std::make_unique<RunHelper>(*dispatcher_, clusterManager(), restarter_, access_log_manager_, init_manager_, overloadManager(),
...
RunHelper::RunHelper(Event::Dispatcher& dispatcher, Upstream::ClusterManager& cm,
  ...
  shutdown_ = false;
  // Setup signals.
  // TODO, may need to enable SIGTERM handler correctly to capture backtraces
#if not defined(__HAS_FF__)
...
// The invocation to RunHelper has been moved up.
void InstanceImpl::run() {
  dispatcher_->run2(Event::Dispatcher::RunType::NonBlock);
}

void InstanceImpl::runHostOnly() {
  // We need the RunHelper to be available to call from InstanceImpl::shutdown() below, so
  // we save it as a member variable.
...
void InstanceImpl::shutdown() {
  ENVOY_LOG(info, "tid [{}] pid [{}] shutdown invoked.",std::this_thread::get_id(), getpid());
  //kill(getpid(), SIGTERM);
